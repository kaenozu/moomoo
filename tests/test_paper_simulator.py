...
def test_buy_sell_and_pnl_flow(tmp_path: Path) -> None:
    ...
    buy = simulator.place_market_order(
    ...
    sell = simulator.place_market_order(
        symbol="US.AAPL", side="SELL", quantity=4.0, price=110.0
    )
...
def test_rejects_insufficient_cash_and_position(tmp_path: Path) -> None:
    simulator = PaperSimulator(
        state_path=tmp_path / "paper-sim-state.json", initial_cash=100.0
    )

    with pytest.raises(ValueError, match="insufficient cash"):
        simulator.place_market_order(
            symbol="US.AAPL", side="BUY", quantity=2.0, price=60.0
        )

    simulator.place_market_order(symbol="US.AAPL", side="BUY", quantity=1.0, price=50.0)
    with pytest.raises(ValueError, match="insufficient position"):
        simulator.place_market_order(
            symbol="US.AAPL", side="SELL", quantity=2.0, price=55.0
        )
...
def test_save_and_load_roundtrip(tmp_path: Path) -> None:
    ...
    simulator.place_market_order(
        symbol="US.MSFT", side="BUY", quantity=5.0, price=200.0
    )
