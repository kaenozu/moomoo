...
def test_recommendation_strategy_initialization() -> None:
    ...
    config = RecommendationConfig(
        symbols=["US.AAPL", "US.MSFT"], top_n=2, weighting_method="equal"
    )
...
def test_recommendation_strategy_waits_for_history() -> None:
    ...
    strategy = RecommendationStrategy(
        RecommendationConfig(symbols=["US.AAPL"], top_n=1, ma_short=20, ma_long=50)
    )
...
def test_recommendation_strategy_score_weighting() -> None:
    ...
    assert decision.target_weights["US.AAPL"] + decision.target_weights[
        "US.MSFT"
