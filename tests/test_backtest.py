...
def test_backtest_returns_metrics() -> None:
    prices, benchmark = make_demo_prices(
        ["US.AAPL", "US.MSFT", "US.NVDA"], periods=260, seed=11
    )
    strategy = MomentumRotationStrategy(
        MomentumRotationConfig(lookback_days=20, trend_days=40, top_n=2)
    )
...
def test_blended_backtest_can_improve_relative_return() -> None:
    prices, benchmark = make_demo_prices(
        ["US.AAPL", "US.MSFT", "US.NVDA", "US.AMZN"], periods=260, seed=11
    )
    strategy = MomentumRotationStrategy(
        MomentumRotationConfig(lookback_days=20, trend_days=40, top_n=2)
    )
...
def test_run_backtest_charges_costs_per_order_level_turnover() -> None:
    class SwitchingStrategy:
        ...
        def decide(self, prices: pd.DataFrame, as_of: pd.Timestamp) -> TradeDecision:
            ...
            self.step += 1
            return TradeDecision(
                as_of=as_of, target_weights=decision, reason=f"step-{self.step}"
            )
