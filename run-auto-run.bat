...
if "%FX_JPY_PER_USD%"=="" (
    "%PYTHON%" -m moomoo_bot.cli auto-run --capital %CAPITAL% --poll-seconds %POLL_SECONDS%
) else (
    "%PYTHON%" -m moomoo_bot.cli auto-run --capital %CAPITAL% --fx-jpy-per-usd %FX_JPY_PER_USD% --poll-seconds %POLL_SECONDS%
)
exit /b %errorlevel%
